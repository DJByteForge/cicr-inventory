...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    if (existing && now < existing.expiresAt) {
      ...
      sendLoginOtpEmail(user.email, user.name, otp).then((r) => {
        if (r.success) {
          console.log(`[AUTH OTP DISPATCH] Success to ${user.email} (msgId: ${(r as any).messageId})`);
        } else {
          console.error(`[AUTH OTP DISPATCH] FAILED to ${user.email}:`, r.error);
        }
      }).catch((e) =>
