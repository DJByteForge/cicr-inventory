...
class AuthManager {
    ...
    private static btnResendLabel: HTMLElement;
    private static btnCancelOtp: HTMLElement;
    private static pendingOtpEmail: string = '';
    private static otpTimerInterval: any = null;
    private static otpSecondsRemaining: number = 600;
    private static resendCooldownSeconds: number = 600;
    private static resendCooldownInterval: any = null;
    ...
    static init() {
        ...
        this.btnResendLabel = document.getElementById('btn-resend-label') || this.btnResendOtp;
    ...
    private static async handleLogin() {
        ...
        try {
            ...
            if (res.ok && data.token) {
                ...
                ToastManager.show('Security Notice Sent', 'An autogenerated sign-in security notice (valid for 10 minutes) was dispatched to your email.', 'info');
                return;
            }
    ...
    private static showOtpModal(email: string, _name?: string, cooldownSeconds: number = 600) {
    ...
    private static startResendCooldown(seconds: number = 15) {
        ...
        const updateResendButton = () => {
            if (!this.btnResendOtp) return;
            if (this.resendCooldownSeconds > 0) {
                const mins = Math.floor(this.resendCooldownSeconds / 60).toString().padStart(2, '0');
                const secs = (this.resendCooldownSeconds % 60).toString().padStart(2, '0');
                if (this.btnResendLabel) {
                    this.btnResendLabel.innerText = `Resend Code (${mins}:${secs})`;
                }
                this.btnResendOtp.setAttribute('disabled', 'true');
                this.btnResendOtp.style.pointerEvents = 'none';
                this.btnResendOtp.style.opacity = '0.5';
                this.btnResendOtp.style.cursor = 'not-allowed';
                this.resendCooldownSeconds--;
            } else {
                clearInterval(this.resendCooldownInterval);
                this.resendCooldownInterval = null;
                if (this.btnResendLabel) {
                    this.btnResendLabel.innerText = 'Resend Code';
                }
                this.btnResendOtp.removeAttribute('disabled');
                this.btnResendOtp.style.pointerEvents = 'auto';
                this.btnResendOtp.style.opacity = '1';
                this.btnResendOtp.style.cursor = 'pointer';
            }
        };

        updateResendButton();
        this.resendCooldownInterval = setInterval(updateResendButton, 1000);
    }
